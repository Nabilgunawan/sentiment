"""
training/emotion_train.py
-------------------------
Script pelatihan (fine-tuning) model klasifikasi emosi Indonesia (7 kelas) berbasis Transformer.
"""
...
def main():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default=str(PROJECT_ROOT / "training" / "datasets" / "emotion_dataset.csv"),
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(PROJECT_ROOT / "models" / "emotion_model"),
        help="Direktori hasil export model"
    )
    parser.add_argument(
        "--metrics_file",
        type=str,
        default=str(PROJECT_ROOT / "training" / "outputs" / "emotion_metrics.json"),
    ...
    # 2. Validate Labels
    # Emotion labels: Marah, Senang, Sedih, Takut, Jijik, Terkejut, Netral
    labels = [str(l).strip() for l in labels]
    valid_labels = config.EMOTION_LABELS
    ...
    config.EMOTION_MODEL_NAME = args.model_name
    config.EMOTION_FINETUNED_PATH = args.output_dir
    ...
    # 4. Fine-Tuning
    logger.info("Memulai pelatihan model emosi...")
    trainer = ModelTrainer("emotion")
    ...
    # 5. Evaluate Model
    logger.info("Memulai evaluasi model emosi...")
    evaluator = ModelEvaluator("emotion")
    ...
    # Write metrics.json inside metrics.json in outputs AND inside the model directory (emotion_model/metrics.json)
    ...
    logger.info(f"Model emosi berhasil dilatih dan disimpan ke {args.output_dir}")
